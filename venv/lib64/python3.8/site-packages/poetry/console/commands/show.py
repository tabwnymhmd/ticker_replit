...
lists all packages available."""
    ...
    def display_package_tree(self, io, package, installed_repo):
        ...
        for dependency in dependencies:
            ...
            info = "{tree_bar}â”€â”€ <{color}>{name}</{color}> {constraint}".format(
                tree_bar=tree_bar,
                color=color,
    ...
    def _display_tree(
        ...
        installed_repo,
        packages_in_tree,
        previous_tree_bar="â”œ",
        level=1,
    ):
        previous_tree_bar = previous_tree_bar.replace("â”œ", "â”‚")
        ...
        dependencies = sorted(dependencies, key=lambda x: x.name)
        tree_bar = previous_tree_bar + "   â”œ"
        i = 0
        total = len(dependencies)
        for dependency in dependencies:
            i += 1
            ...
            info = "{tree_bar}â”€â”€ <{color}>{name}</{color}> {constraint} {warn}".format(
                tree_bar=tree_bar,
                color=color,
                name=dependency.name,
                constraint=dependency.pretty_constraint,
