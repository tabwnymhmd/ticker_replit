...
async def test_idna(monkeygai):
    # This is the encoding for "faÃŸ.de", which uses one of the characters that
    # IDNA 2003 handles incorrectly:
    monkeygai.set("ok faÃŸ.de", b"xn--fa-hia.de", 80)
    ...
    assert "ok faÃŸ.de" == await tsocket.getaddrinfo("faÃŸ.de", 80)
    assert "ok faÃŸ.de" == await tsocket.getaddrinfo("xn--fa-hia.de", 80)
    assert "ok faÃŸ.de" == await tsocket.getaddrinfo(b"xn--fa-hia.de", 80)
...
async def test_custom_hostname_resolver(monkeygai):
    ...
    got = await tsocket.getaddrinfo("fÃ¶Ã¶", "foo")
