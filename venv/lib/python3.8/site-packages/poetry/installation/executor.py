...
class Executor(object):
        ...
        try:
            if self.supports_fancy_output():
                    ...
                    if self._should_write_operation(operation):
                        with self._lock:
                            ...
                            self._sections[id(operation)].write_line(
                                "  <fg=blue;options=bold>â€¢</> {message}: <fg=blue>Pending...</>".format(
                ...
                if self._should_write_operation(operation):
                    if not operation.skipped:
                        self._io.write_line(
                            "  <fg=blue;options=bold>â€¢</> {message}".format(
                                message=self.get_operation_message(operation),
                            ),
                        )
                    else:
                        self._io.write_line(
                            "  <fg=default;options=bold,dark>â€¢</> {message}: "
            ...
            try:
                ...
                if not self.supports_fancy_output():
                    ...
                    message = "  <error>â€¢</error> {message}: <error>Failed</error>".format(
            ...
            try:
                message = "  <warning>â€¢</warning> {message}: <warning>Cancelled</warning>".format(
    ...
    def _do_execute_operation(self, operation):
        ...
        message = "  <fg=green;options=bold>â€¢</> {message}".format(
    ...
    def _execute_uninstall(self, operation):  # type: (Uninstall) -> None
        message = "  <fg=blue;options=bold>â€¢</> {message}: <info>Removing...</info>".format(
            message=self.get_operation_message(operation),
        )
        self._write(operation, message)
    ...
    def _install(self, operation):
        ...
        operation_message = self.get_operation_message(operation)
        message = "  <fg=blue;options=bold>â€¢</> {message}: <info>Installing...</info>".format(
            message=operation_message,
        )
        self._write(operation, message)
    ...
    def _prepare_file(self, operation):
        ...
        message = "  <fg=blue;options=bold>â€¢</> {message}: <info>Preparing...</info>".format(
            message=self.get_operation_message(operation),
        )
        self._write(operation, message)
    ...
    def _install_directory(self, operation):
        ...
        message = "  <fg=blue;options=bold>â€¢</> {message}: <info>Building...</info>".format(
            message=operation_message,
        )
        self._write(operation, message)
    ...
    def _install_git(self, operation):
        ...
        message = "  <fg=blue;options=bold>â€¢</> {message}: <info>Cloning...</info>".format(
            message=operation_message,
        )
        self._write(operation, message)
    ...
    def _download_archive(self, operation, link):  # type: (Operation, Link) -> Path
        ...
        operation_message = self.get_operation_message(operation)
        message = "  <fg=blue;options=bold>â€¢</> {message}: <info>Downloading...</>".format(
            message=operation_message,
        )
        progress = None
        if self.supports_fancy_output():
